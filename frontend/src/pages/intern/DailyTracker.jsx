...
export default function InternDailyTracker() {
  ...
  const handleSubmit = async (e) => {
    ...
    if (form.aiCredits !== '' && (Number(form.aiCredits) < 0 || Number(form.aiCredits) > 1000)) {
      toast.error("AI Credits must be between 0 and 1000");
      return;
    }
    setSaving(true);
    try {
        ...
        <form onSubmit={handleSubmit} className="bg-white border border-[#E2E8F0] rounded-xl shadow-sm p-6 space-y-5">
          ...
          <div className="grid grid-cols-1 sm:grid-cols-3 gap-4">
            ...
            <div>
              ...
              <input type="number" min="0" max="1000" value={form.aiCredits} onChange={set('aiCredits')} placeholder="0" className="w-full border border-[#E2E8F0] rounded-md py-2 px-3 text-[13px] focus:outline-none focus:border-[#2563EB]" />
            </div>
          </div>
