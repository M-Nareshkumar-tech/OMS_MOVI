...
const validateAndEnrich = (rawRows, roles, departments) => {
  return rawRows.map((row, i) => {
    ...
    const empId      = row[norm('employeeId')] || row[norm('employee id')] || row[norm('internId')] || row[norm('intern id')] || '';
...
export default function BulkImportModal({ isOpen, onClose, onComplete }) {
  ...
  const downloadTemplate = () => {
    ...
    const lines = [
      'name,email,employeeId,internId,phone,designation,department,role,employmentType,college,domain,batch',
                ...
                <div className="flex items-center justify-between gap-4">
                  <div className="min-w-0">
                    ...
                    <p className="text-[12px] text-[#64748B] mt-0.5">
                      ...
                      {' · '}Optional: employeeId, internId, phone, designation, employmentType
                    </p>
                  </div>
                  <button
