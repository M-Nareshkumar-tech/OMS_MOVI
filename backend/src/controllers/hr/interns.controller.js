...
export const getInternRequests = async (req, res, next) => {
  try {
    const requests = await InternRequest.find()
...
export const updateInternRequestStatus = async (req, res, next) => {
  try {
    ...
    // Send notification to the PMO lead who made the request
    await sendNotification({
      recipient: request.requestedBy,
      type: 'system_alert',
      title: 'Intern Request Status Update',
      message: `Your intern request for project allocation has been marked as "${status}".`,
      link: '/pmo/interns',
      sender: req.user._id,
    });

    sendSuccess(res, request, `Request marked as ${status}`);
  } catch (error) {
    next(error);
  }
};
