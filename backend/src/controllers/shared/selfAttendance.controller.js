...
export const checkOut = async (req, res, next) => {
  try {
    ...
    const parseTime = (str) => {
      if (!str) return new Date(record.date || today);
      
      // Check if it is an ISO or absolute date string
      if (str.includes('T') || str.includes('GMT') || str.includes('-') || (!isNaN(Date.parse(str)) && !str.includes(':'))) {
      ...
      const isPM = normalizedStr.endsWith('PM');
      const isAM = normalizedStr.endsWith('AM');
      
      // Strip AM/PM suffix
      const timePart = normalizedStr.replace('AM', '').replace('PM', '').trim();
      ...
      const d = new Date(record.date || today);
      d.setHours(h, m, 0, 0);
      return d;
    };
